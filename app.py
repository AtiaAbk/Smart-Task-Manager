...
task_name = st.text_input("Task Name", key=f"input_name_{fk}")
task_date = st.text_input("Date (YYYY-MM-DD or DD/MM/YYYY)", key=f"input_date_{fk}")
task_time = st.text_input("Time (HH:MM or HH:MM AM/PM)", key=f"input_time_{fk}")
...
""")
    ...
    with col1:
        if not task["done"]:
            if st.button("✅ Mark Done", key=f"d{i}_{fk}"):
                st.session_state.tasks[i]["done"] = True
                st.rerun()
    with col2:
        if st.button("🗑️ Delete", key=f"x{i}_{fk}"):
